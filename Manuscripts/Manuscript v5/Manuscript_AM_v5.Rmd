...
m_1 <- gam(StO2_sim ~ Group+s(Day, by = Group, k = 5),method='REML', data  = dat_sim)
...
f_predict<-f_predict%>%
    mutate(fit = predict(m_1,f_predict,se.fit = TRUE,type='response')$fit,
           se.fit = predict(m_1, f_predict,se.fit = TRUE,type='response')$se.fit)
