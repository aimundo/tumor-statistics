...
\begin{equation}
\eta_{ijt}=\beta_0+\beta_1 \times time_{t} +\beta_2 \times treatment_{j} +\beta_3 \times time_{t}\times treatment_{j},
\label{eq:GLM-eta}
\end{equation}
...
\begin{equation}
\eta_{ijt}=\beta_0+\beta_1 \times time_{t} +\beta_2 \times treatment_{j} +\beta_3 \times time_{t}\times treatment_{j}+\alpha_{ij},
\label{eq:GLMM-eta}
\end{equation}
...
Where \(\beta_{0}\) is the intercept, \(\beta_{1}\) is the coefficient for each treatment group. Notice that we and \(f(time_{t}|\beta_{j})\) represents the smooth function of time, with coefficient \(\beta_{j}\). A GAM version of a linear model can be written as:
...
Where \(y_{ijt}\) is the response at time \(t\) of subject \(i\) in group \(j\), \(\beta_0\) is the expected value at time 0, and the change of \(y_{ijt}\) over time (instead of being defined by \(\beta_{j}time_{ijt}\) as in Equation \eqref{eq:GLM-eta}) is represented by the smooth function \(f(time_t\mid \beta_j)\) with inputs as the covariates \(x_t\) and parameters \(\beta_j\), and \(\varepsilon_{ijt}\) represents the deviation of each observation from the mean.
