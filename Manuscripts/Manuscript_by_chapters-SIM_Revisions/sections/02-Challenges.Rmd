# The contemporary statistical landscape

Although the limitations of rm-ANOVA and LMEMs to analyze non-linear trends in longitudinal data have been presented in a general way in the previous section, a proper understanding of the matter would be incomplete without a brief description of the current accepted knowledge regarding statistical models. The advances over the last two decades in computational power and the theoretical framework for statistical models have significantly changed the hierarchy of statistical models. Because the topic itself is far outside of the scope of this work

## Linear Models (LMs) and Linear Mixed Models (LMEMs)

Linear models (LMs) are those that assume a normal (Gaussian) distribution, such as ANOVA. These are by far the models most commonly used to analyze data within the biomedical research community, and only incorporate _fixed effects_. On the other hand, Linear Mixed Effect Models (LMEMs) are those that still assume normality but also incorporate _random effects_, as it has been described before. 

## Generalized Linear Models (GLMs)

A major limitation of LMs is their assumption of normalit, and although it can be forced by the use of a transformation, this procedure can lead to poor model performance [@ohara2010], and can cause problems with the biological interpretation of the model estimates. McCullagh and Nelder [@nelder1972] defined General Linear Models (GLMs) as an extension of LMs, where the data does not need to be normally distributed. To achieve this, consider the following equations:
...
Where $y_{ijt}$ is the observed response, which is assumed to follow some distribution $\mathcal{D}$ with some mean $\mu_{ijt}$ and a dispersion parameter $\phi$. The mean $\mu_{ijt}$ is also known as the _expected value_ ($E$) of the observed response.
...
Need the two first equations 


and this one

\begin{equation}
$\eta_{ijt}=\beta_0+\beta_1x_{ijt}$ 
(\#eq:GLM-eta)
\end{equation}
...
### The Linear Mixed Model (LMEM) Case 
...
Of notice, LMEMs are capable of fitting non-linear trends using an "empirical" approach, which is described in detail by Pinheiro and Bates [@pinheiro2006] using polynomial fixed effects instead of linear effects such as in Equation \@ref(eq:LMEM). However, polynomial fits have limited predictive power and cause bias on the boundaries of the covariates [@beck1998]; more importantly, polynomial effects lack biological or mechanistic interpretation [@pinheiro2006] which limits their use in biomedical studies.
