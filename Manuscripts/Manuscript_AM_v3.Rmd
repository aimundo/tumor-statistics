...
To demonstrate the limitations of rm-ANOVA for non-linear longitudinal data, this section presents a simulation experiment of a normally distributed response of two groups of 10 subjects each. To each group, an rm-ANOVA model (Equation \@ref(eq:linear-model)) is fitted using $\textsf{R}$[@r] and the package _nlme_[@nlme] (code in the Appendix). 
Briefly, two cases for the mean responses for each group are considered: in the first case, the mean response in each group is a linear function with different intercepts and slopes; a negative slope is used for Group 1 and a positive slope is used for Group (Figure \@ref(fig:linear-models, A)) .In the second case, a second-degree polynomial (quadratic) function is used for the mean response per group: the quadratic function is concave down for Group 1 and it is concave up for Group 2 (Figure \@ref(fig:quadratic-response),A). From the mean response per group (linear or quadratic), individual responses for each subject within each group are simulated using covariance matrices with compound symmetry (constant variance across time) or independent errors (errors that are generated from a a normal distribution that are not constant over time). Thus, the response per subject in both the linear and quadratic simulation corresponds to the mean response per group plus the error (either compound symmetry or independent, which appear in panels A and E, respectively in both Figure \@ref(fig:linear-models) and Figure \@ref(fig:quadratic-response)) . To this data, an rm-ANOVA model identical as Equation  \@ref(eq:linear-model) is fitted. The resulting simulated data, simulated errors, individual response and fitted parameters from the model are plotted.
