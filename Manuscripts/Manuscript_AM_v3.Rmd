...
To further clarify the concept of basis functions and smooth functions, consider the simulated response for Group 1 in Figure @\ref(fig:quadratic-response). The simplest GAM model that can be used to estimate such response is that of a single smooth term for the time effect; i.e., a model that fits the trend trough the timeline.  Because there are six timepoints for this group, the use of five equally spaced knots ensures that the model will not have computational issues. Asking the model to use five knots to construct the smooth term means that it will have _four_ effective basis functions (as one will be used for the intercept). The choice of basis functions is already optimized in the package _mgcv_ depending on the number of knots. In Panel A of Figure \@ref(fig:basis-functions-plot), the four basis functions are shown. Each of the basis functions is composed of six different points (because there are six points on the timeline). To control the wigliness of the fit, each of the basis functions of Panel A is penalized, or multipled by a coefficient according to the penalty matrix of Panel B. In other words, the six points of each basis will be multiplied by the corresponding coefficient, thereby increasing or decreasing the original basis functions of Panel B. In Figure \@ref(fig:basis-functions-plot), Panel C shows the resulting penalized basis functions. Note that the penalizetion for basis 1 has resulted in a decrease of its overall value (because the coefficient for that basis function is negative and less than 1); on the other hand, basis 3 has increased its value roughly 2.5 times. Finally, the penalized basis functions are added at each timepoint to produce the final value of the smooth term. The resulting smooth term for the effect of _time_ is shown in Panel D (orange line) along the simulated values per group, which appear as points.

```{r,basis-functions-plot,fig.width=16, fig.height=12,  echo=FALSE,message=FALSE,fig.show='hold',fig.cap="**Basis functions for a single smoother for time with five knots. A: Basis functions for a single smoother for _time_ for the simulated data of Group 1 from Figure 2, the intercept basis is not shown. B: Penalty matrix for the basis functions. Each basis function is penalized by a coefficient which can be positive or negative. The coefficient determines the overall effect of each basis in the final smoother. C: Penalized basis functions. Each of the four basis functions of panel A has been penalized by the corresponding coefficient shown in Panel B, note the corresponding increase (or decrease) of each basis. D. Smoother for _time_ and original datapoints. The smoother (line) is the result of the sum of each penalized basis function at each time point, simulated values for the group appear as points.**"}
...
basis_plot$Basis<-rep(1:4,each=6) #pasting basis number values
basis_plot$value_orig<-basis$value #pasting basis values
basis_plot$cof<-rep(coef(gm)[2:5],each=6) #pasting coefficients
...
#basis multiplied by the coefficient
...
#plotting simulated datapoints and spline  
...
#Combining all
p11+p13+p12+p14+plot_annotation(tag_levels='A')&
  theme(plot.title = element_text(size = 30, 
                                face = "bold"),
     text=element_text(size=30))
