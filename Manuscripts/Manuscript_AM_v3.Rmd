...
#original bases
p1<-ggplot(basis_plot,aes(x=time,y=value_orig,colour=factor(Basis)))+
...
p2<-ggplot(basis_plot,aes(x=time,y=mod_val,colour=factor(Basis)))+
  geom_line()+geom_point()+
  facet_grid(~Basis)+
  theme_classic()+
  scale_color_aaas()
...
p3<-ggplot(basis_plot,aes(x=Basis,y=Basis,fill=cof))+
  geom_tile(aes(color='black'),size=2)+
  geom_tile(size=2)+
