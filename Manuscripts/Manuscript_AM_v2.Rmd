...
\begin{equation}
y_{ijt} = \mu+ \beta_1* time_{t} + \beta_2* treatment_{j} + \beta_3* time_{t}*treatment_{j}+\pi_{ij} +\varepsilon_{tij}\\ 
(\#eq:linear-model)
\end{equation}
...
\begin{equation}
y_{ijt} = \begin{cases}
\mu + \beta_1*time_{t}+\varepsilon_{ijt}+\pi_{ijt}   & \mbox{if Group A}\\
\mu + \beta_1 * time_{t} + \beta_2 * treatment_{j} +\beta_3* time_{t} * treatment_{j}+\pi_{ijt}+\varepsilon_{ijt}  & \mbox{if Group B}\\
\end{cases}
(\#eq:ANOVA-by-group)
\end{equation}
...
y_{ijt} = \begin{cases}
\mu + \beta_1*time_{t}+\varepsilon_{ijt}+\pi_{ijt}   & \mbox{if Group A}\\
\widetilde{mu} + \widetilde{\beta_1} * time_{t} +\pi_{ijt}+\varepsilon_{ijt}  & \mbox{if Group B}\\
\end{cases}
(\#eq:ANOVA-lines)
\end{equation}
...
plot_example(example(fun_type = "linear", error_type = "correlated")) +
  ...
  title = 'Simulated with linear function and correlated errors',
    theme = theme(plot.title = element_text(size = txt),text=element_text(size=txt))
)
```
...
The model specification for GAMs requires that the _smooth functions_ are represented in a parametric way that fit within the GLM framework, and this step is achieved by using a _basis function_ to represent such functions. A _basis_ is a set of functions that capture the space where the smooths that approximate $f(x_t \mid \beta)$ exist [@simpson2018]. In other words, 
