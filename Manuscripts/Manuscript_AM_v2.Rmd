...
plot_example <- function(sim_dat) {
  ...
  p2 <- sim_dat$dat %>%
    ...
    geom_line(aes(x = time, y = mu, color = treatment, size = "Simulated Truth"), lty = 1,show.legend = FALSE) +
   ...
   p3 <- sim_dat$dat %>%
    ...
    geom_line(show.legend=FALSE) +labs(y='response')+
     theme_classic()+
    # facet_wrap(~ treatment) +
...
In this case, a comparison of the predictions of the rm-ANOVA model to the the simulated data and indicates that the model is not capturing the changes within each group in a good way. This is expected as the rm-ANOVA model is only able to accommodate linear changes, which in this case fail to capture the changes over the timeline. Specifically, note that in Figure \@ref(fig:quadratic-response) panels D and H the model is not capturing the increase in the response through time points 2 and 4. If such incrase may indicate a physiologically relevant change, the model will not be able to capture it, potentially limiting inferences from the data.
