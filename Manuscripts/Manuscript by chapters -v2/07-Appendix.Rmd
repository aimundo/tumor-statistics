...
```{r, plotting layouts,echo=TRUE, include=TRUE}
...
This section covers the code used to generate Figure \@ref(fig:sim-smooth-plot), where the simulated data, fit of the "final" GAM (`m1`), linear model  and GAM on data with missing observations are presented. Note that panel A in Figure \@ref(fig:sim-smooth-plot) and the inlet are generated in the code chunk where the data is simulated in Section \@ref(tumor-data-simulation), and are called later to build the figure.
...
```{r,pairwise-comp-Appendix,echo=TRUE,include=TRUE,message=FALSE,warning=FALSE}
##Pairwise comparisons
...
(ref:pairwise-caption-both) Pairwise comparisons for smooth terms. A: Pairwise comparisons for the full dataset.  B: Pairwise comparisons for the dataset with missing observations. Significant differences exist where the interval does not cover 0. In both cases the effect of treatment is significant after day 5.

```{r,plot-pairwise-comp-Appendix,fig.width=10, fig.height=7, out.width='75%', fig.align='center',echo=FALSE,message=FALSE, warning=FALSE,fig.cap='(ref:pairwise-caption-both)'}
