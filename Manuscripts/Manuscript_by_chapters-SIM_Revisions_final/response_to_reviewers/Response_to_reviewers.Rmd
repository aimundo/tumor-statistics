...
- L504-510 now indicate that we are computing a simultaneous CI around the difference, and that we _included_ the group means in our pairwise comparison. We also indicate to the reader the fact that if multiple parametric terms exist in the model the inclusion of the means can become problematic. 
...
The sentence now reads "separate smooths for..." (L441 in the revised manuscript)
...
We use the code notation provided by the $LaTeX$ package `listings`, and it provides curly quotes for code. Regarding `family`, we believe that it is not necessary to include it here, but we indicate in L459-460 what the argument exists and that other distributions can be used.
...
For the length of the covariates explored in this paper TPRS are sufficient. However, we want to make clear to the reader that other types of splines that might be appropriate in certain situations are available. To acknowledge this, L453 in the updated manuscript now reads "(a description of all the available smooths can be found by typing `?mgcv::smooth.terms` in the Console)".
...
The word "total" has been removed. The sentence now reads "consider the random deletion of 40% of the..." (L474)
...
The sentence now reads "...with as few as 4 observations per group..." (L482)
...
The sentence has been rewritten. It now reads "...the observed difference is significant..." (L494).
...
We have made the appropriate changes. The sentence now reads "The GAM model is therefore able to identify changes between the groups at time points..." (L557)
