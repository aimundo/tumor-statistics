...
A linear mixed model for a longitudinal study can be written as:
...
When Equation \@ref(eq:linear-model) and Equation \@ref(eq:LMEM) are compared, it is apparent that the LMEM and rm-ANOVA models have the same construction regarding the _fixed effects_ of time and treatment, but that the LMEM incorporates an additional source of variation (the term $\mu_{ij}$). This term is defined as a _random effect_, which accounts for variability in each subject within each group. The _random_ component can be defined as "noise" caused by some inherent variability within the study that is not captured by the term $\varepsilon_{tij}$. 
