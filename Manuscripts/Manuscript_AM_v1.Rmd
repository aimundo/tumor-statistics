...
Mathematically speaking, a linear model can be written as:
...
The rm-ANOVA model  can be re-written under the generalized linear model  (GLM) framework [@nelder1972]. The main difference in this case is that a GLM model  uses a _linking function_ to connect the observed value to the linear model. Thus, 
...
\begin{equation}
\label{eq:ANOVA_GLM}
$$y_{kit}=\mu+f(\gamma_k)+f(\tau_t)+(\gamma \tau)_{kt}+\pi_{i(k)}+e_{kit}$$
\end{equation}
