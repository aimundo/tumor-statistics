...
The resulting model is model `gam1`, which is the model fitted in the main manuscript. By using `appraise()` and `draw` on this model (Figure  \@ref(fig:final-GAM-diag)) we see that the trend on the QQ plot has improved, the histogram of the residuals appears to be reasonably distributed, and the smooths are capturing the trend of the data within each group . From `gam.check`, the k-index is now at an acceptable value (~1.02), and `summary` now indicates that the model is able to capture 87% of the variance data.
...
(ref:pairwise-comp-caption) Smooth pairwise comparisons using a 95% confidence interval for the difference between smooths. 

```{r, pairwise-comp-workflow-fig, fig.width=10, fig.height=7, out.width='75%', fig.align='center',echo=FALSE,message=FALSE, warning=FALSE,fig.cap='(ref:pariwise-comp-caption)'}
c1
```

Of notice, a conveniente wrapper for the function described above exists as `difference_smooths` in the package `gratia`. It produces the same plot when the function is used on a fitted model. The function syntax can be checked at:
...
```{r, plotting-appendix, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE}
#linear model
...
```{r,pairwise-comp-Appendix,echo=FALSE,include=FALSE,message=FALSE,warning=FALSE}
##Pairwise comparisons
...
```{r,plot-pairwise-comp-Appendix,fig.width=10, fig.height=7, out.width='75%', fig.align='center',echo=FALSE,message=FALSE, warning=FALSE,fig.cap='(ref:pairwise-comp-caption)'}
